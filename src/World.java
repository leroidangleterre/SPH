...
public class World{
    ...
    // Angle of elementary rotation of all rectangles.
    private double rectangleRotation = Math.PI / 16;
...
//        this.setBorderParticles();
    ...
    private void evoluer(double dt){
        ...
        try{
            ...
            for(int i = 0; i < this.nbLines; i++){
                this.getSquare(i, 0).influenceMeanSpeed(45.0, 0.0);
            }
...
//        for(ArrayList<Square> line : this.tab){
//            for(Square s : line){
//                for(int i = 0; i < s.getParticlesAmount(); i++){
...
//        }
    ...
    public void rotateAllRectangles(boolean clockwise){
        double angle;
        if(clockwise){
            angle = -this.rectangleRotation;
        } else{
            angle = this.rectangleRotation;
        }
        this.rotateAllRectangles(angle);
    }
    ...
    public void setBorderParticles(){
        double dx = 1;
        double x = xMin;
        double x2 = this.getSquare(0, this.nbColumns - 1).getXMax();
        double y = yMin + 0.01;
        Square target;
        do{
            target = this.getSquareFromCoordinates(xMin, y);
            if(target != null){
                target.createParticle(x, y);
            }
            target = this.getSquareFromCoordinates(xMax - 0.1, y);
            if(target != null){
                target.createParticle(xMax, y);
            }
            y += dx;
        } while(y <= yMax);
        x = xMin;
        do{
            target = this.getSquareFromCoordinates(x, yMin + 0.01);
            if(target != null){
                target.createParticle(x, yMin + 0.1);
            }
            x += dx;
        } while(x <= xMax);
